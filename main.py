...
def analyze_and_generate(crawled_text):
    ...
    system_prompt = f"""
    你是首席零售与跨境物流战略分析师。今天是 {today_str}，昨天是 {yesterday_str} (T-1)。
    ...
    ## 📈 第一部分：宏观大盘与外单指数 (请以表格呈现)
    | 指数板块 | 核心指标名称 (如: SCFI/社零增速) | 最新动态/数据估值 | 战略简评 |
    | :--- | :--- | :--- | :--- |
    | **跨境与外单** | ... | ... | ... |
    | **国内零售** | ... | ... | ... |
    | **干线与末端** | ... | ... | ... |
    
    ## 🗞️ 第二部分：核心阵地动态监测 (T-1 重点聚焦)
    *(提炼 8-10 条最具战略价值的去重资讯)*
    * **[标签，如: 跨境出海 / 仓配一体]**：**事件核心摘要** 
      * *AI洞察*：[结合物流打法分析其战略意图]
      * *来源*：[原文链接](提取出的实际URL)
    * *(以此类推)*
